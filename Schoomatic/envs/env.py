...
from envs.gplan import compute_route_waypoints
...
class Env(gym.Env):
        ...
        while True:
            try:
                ...
                print("time-out of "+str(self.timeout)+"000ms while waiting for the simulator, make sure "
                                                       "the simulator is ready and connected to "+str(self.host)+":"+str(self.port))
            else:
                break
        ...
        if self.src_loc:
            ...
            for i in range(0, len(spawn_transforms)):
                ...
                if vehicle.actor is not None:
                    ...
                    if len(self.route_waypoints) < 100:
                        continue
    ...
    def render(self, mode="human"):
        if self.display:
            ...
            self.display.blit(pygame.surfarray.make_surface(self.observation[:,:,0].swapaxes(0, 1)), (0, 0))
            # Superimpose current observation into top-right corner
            self.display.blit(pygame.surfarray.make_surface(RGB_img.swapaxes(0, 1)),
