...
def distance(a, b):
    return math.sqrt((a[0]-b.x)**2 + (a[1]-b.y)**2)
...
class CarlaLapEnv(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        if self.src_loc:
            #self.route_waypoints = [[self.src_loc[0],self.src_loc[1]], [-26, 29], [36, 27.5], [43.2, 62.6], [98.3, 74.4], [88.6, 121.6], [-31.6, 132]]
    ...
    def render(self, r, l, mode="human"):
        if self.display:
            # Add metrics to HUD
            ...
            self.display.blit(pygame.surfarray.make_surface(self.info['rgb_obs'].swapaxes(0, 1)), (0, 700))
            r = cv2.resize(r, (10, 10), interpolation=cv2.INTER_LINEAR)
            _, r = cv2.threshold(r, 100, 255, cv2.THRESH_BINARY)
            r = cv2.resize(r, (64, 64), interpolation=cv2.INTER_LINEAR)
            self.display.blit(pygame.surfarray.make_surface(r.swapaxes(0, 1)), (0, 400))
            self.display.blit(pygame.surfarray.make_surface(l.swapaxes(0, 1)), (0, 500))
    ...
    def _transform(self, wp_loc):
        ...
        dis_v = np.arctan2(wp_loc[1]-location.y, wp_loc[0]-location.x)
    ...
    def _reward_fn(self):
        ...
        if self.goal_reward == "propdist":
            if self.sparse_reward_fn:
                wp_loc = self.route_waypoints[-1]
            else:
                wp_loc = self.route_waypoints[1]

        self._transform(wp_loc)
    ...
    def _draw_path(self, life_time=60.0, skip=0):
        ...
        w0 = carla.Location(x=self.route_waypoints[1][0], y=self.route_waypoints[1][1], z=0.1)
